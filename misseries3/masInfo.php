...
<body>
	...
	<?php foreach($resultado as $item) { ?>
		...
		<?php 
			...
			if(isset($_GET['star'])) {
				$t=serie::actualizarPuntuacion($_GET["id"],$_GET['star']);
				$id=intval($_GET['id']);
				header("Location: masInfo.php?id=$id");
			}
		?>
			...
			<div id="content">
				...
				<p>Puntuacion: 
					<?php 
						...
						for($i=1;$i<=intval($item["puntuacion"]);$i++) {
							?>
								<a href="masInfo.php?id=<?= $_GET["id"] ?>&idUsu=<?= $_SESSION['usuario'] ?>&star=<?= $i ?>"><i class="bi bi-star-fill" id="<?= $i ?>"></i></a>
							<?php
						}

						for($i=intval($item["puntuacion"])+1;$i<=10;$i++) {
							?>
								<a href="masInfo.php?id=<?= $_GET["id"] ?>&idUsu=<?= $_SESSION['usuario'] ?>&star=<?= $i ?>"><i class="bi bi-star" id="<?= $i ?>"></i></a>
							<?php
						}
					?>
				</p>
